...
def _make_tech_chart(ticker: str, hist: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=list(bb_u.ffill()) + list(bb_l.ffill()[::-1]),
            fill="toself",
...
def _monte_carlo_chart(
    ...
    # Strip timezone so bdate_range and hist index are both tz-naive for Plotly
    last_dt_naive = last_dt.tz_localize(None) if hasattr(last_dt, "tz_localize") and last_dt.tzinfo else last_dt
    future_dates  = pd.bdate_range(start=last_dt_naive, periods=horizon + 1)[1:]

    fig = go.Figure()
    # Historical (last 60 days) — also strip tz
    hist_plot = close.iloc[-60:]
    hist_index_naive = hist_plot.index.tz_localize(None) if hist_plot.index.tzinfo else hist_plot.index
    fig.add_trace(go.Scatter(
        x=hist_index_naive, y=hist_plot.values,
    ...
    fig.add_vline(x=str(last_dt_naive.date()), line_dash="dot",
